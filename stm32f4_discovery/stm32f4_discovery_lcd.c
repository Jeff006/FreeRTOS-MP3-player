...
void LCD_CtrlLinesConfig(void)
{
	...
	/* LCD Vsync configuration */
	GPIO_InitStructure.GPIO_Pin = LCD_VSYNC_PIN;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_2MHz;
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
	GPIO_InitStructure.GPIO_PuPd  = GPIO_PuPd_NOPULL;

	GPIO_Init(LCD_VSYNC_PORT, &GPIO_InitStructure);
}
...
{ 
  ...
  SSD2119Init(void)
	...
	//LCD_WriteReg(SSD2119_PWR_CTRL_1_REG, 0x6AF4);
	...
	LCD_WriteReg(SSD2119_ENTRY_MODE_REG, 0xE030);
	...
	/* Set frame rate */
	LCD_WriteReg(SSD2119_FRAME_CYCLE_CTRL_REG, 0x4310);//0x5310
	...
	LCD_WriteReg(SSD2119_VERT_PO_CTRL_REG, 0x0000);
...
void LCD_DisplayStringLine(uint16_t Line, uint8_t *ptr)
{
	...
	GPIO_SetBits(LCD_VSYNC_PORT, LCD_VSYNC_PIN);
	...
	GPIO_ResetBits(LCD_VSYNC_PORT, LCD_VSYNC_PIN);
}
